...
### Livewire: dropped noise and legible failures

Livewire/Filament apps generate a family of browser captures that are either pure noise or unreadable, so the helper ships a `beforeSend` for them. It lives in a second published file, `resources/js/vendor/livewire.js`, kept import-free so it is unit-testable — the package tests it with `composer test:js` (`node --test`, no npm dependency).

**Dropped** — transient, never a real bug: a 419 page-expired rejection (idle session / stale CSRF); a `wire:navigate` transition the user clicked through (`isFromCancelledTransition`); a component torn down mid-navigation (`Component not found: undefined` — a *named* missing component still reports); and a `{status, body, json, errors}` response with every key null, which means the request was superseded or aborted before it settled rather than failed.

**Made legible** — everything else with those four keys. Livewire rejects a failed request with its raw response object instead of an `Error`, so Sentry can only title it *"Object captured as exception with keys: …"* and fingerprint on that key list: a 502 on a lazy-load and a 500 on a table filter collapse into one meaningless issue, with the status buried in `extra.__serialized__`. Worse, the stack is entirely the minified SDK's own capture machinery, so the issue looks like *Watchtower itself* is crashing. The hook restates these as `LivewireRequestFailed: Livewire request failed with HTTP <status>` and fingerprints by status, giving each failure mode its own issue. It filters nothing — it only surfaces data the event already carries.

Because the fingerprint changes, any pre-existing "Object captured as exception with keys: …" issue stops receiving events after you deploy this; resolve it once the replacement issues appear.

The published helper is yours to edit — `--force=false` on re-runs means customizations are preserved. The package version is the source of truth in `vendor/phattarachai/watchtower-laravel/resources/js/{watchtower,livewire}.js`. To customize the Sentry options (e.g. add `ignoreErrors`), edit `Sentry.init({ ... })` inside the published helper, not in every entry. To customize the meta-tag template, publish the view with `php artisan vendor:publish --tag=watchtower-views` and edit `resources/views/vendor/watchtower/user-meta.blade.php` — the `@watchtowerUser` directive will pick up the override automatically.
